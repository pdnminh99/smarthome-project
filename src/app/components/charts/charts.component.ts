...
export class ChartsComponent implements OnInit {
  ...
  public set temperature(newTemp: number) {
    ...
    if (
      this.localTemperatures[0].data.length <= 10 &&
      this.localTemperatures[0].data.length > 0
    ) {
      this.temperatureTimeline.push(`${this.temperatureTimeline.length}`);
    } else {
      this.localTemperatures[0].data = this.localTemperatures[0].data.slice(
        1,
        this.localTemperatures[0].data.length
      );
    }
  }
  ...
  public set humidity(newHumidity: number) {
    ...
    if (
      this.localHumidity[0].data.length <= 10 &&
      this.localHumidity[0].data.length > 0
    ) {
      this.humidityTimeline.push(`${this.humidityTimeline.length}`);
    } else {
      this.localHumidity[0].data = this.localHumidity[0].data.slice(
        1,
        this.localHumidity[0].data.length
      );
    }
  }
  ...
  getChartColors(isTemperatureChart: boolean): Color[] {
    return [
      {
        backgroundColor: isTemperatureChart
          ? "rgba(255, 31, 31, 0.2)"
          : "rgba(0, 4, 245, 0.2)",
        borderColor: isTemperatureChart
          ? "rgba(255, 31, 31, 1)"
          : "rgba(0, 4, 245, 1)",
